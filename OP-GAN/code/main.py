...
def parse_args():
    ...
    parser.add_argument('--gpu', dest='gpu_id', type=str, default='0,1') # gpu id = 0,1
...
def get_dataset_indices(split="train", num_max_objects=10):
    ...
    for _i in range(num_max_objects+1):
        dataset_indices.append([]) # object 개수만큼 [] 생성
...
if __name__ == "__main__":
    ...
    if args.resume == "":
        ...
        output_dir = 'OP-GAN/output/%s_%s_%s' % (cfg.DATASET_NAME, cfg.CONFIG_NAME, timestamp)
    else:
    ...
    if cfg.TRAIN.OPTIMIZE_DATA_LOADING:
        ...
        dataset = TextDataset(cfg.DATA_DIR, img_dir, split_dir, base_size=cfg.TREE.BASE_SIZE, transform=image_transform, eval=eval, use_generated_bboxes=cfg.TRAIN.GENERATED_BBOXES)
        assert dataset
    ...
    if cfg.TRAIN.FLAG:
        if not resume:
            copyfile(sys.argv[0], output_dir + "/" + "main.py")
            copyfile("./OP-GAN/code/trainer.py", output_dir + "/" + "trainer.py")
            copyfile("./OP-GAN/code/model.py", output_dir + "/" + "model.py")
            copyfile("./OP-GAN/code/miscc/utils.py", output_dir + "/" + "utils.py")
            copyfile("./OP-GAN/code/miscc/losses.py", output_dir + "/" + "losses.py")
            copyfile("./OP-GAN/code/GlobalAttention.py", output_dir + "/" + "GlobalAttention.py")
            copyfile("./OP-GAN/code/datasets.py", output_dir + "/" + "datasets.py")
